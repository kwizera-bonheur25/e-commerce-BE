...
describe("ORDER API TEST", () => {
	...
	it("should return 404 when the order is not found", async () => {
		const nonExistentOrderId = "non-existent-order-id123";

		// Mock the Order.findOne method to return null, indicating the order was not found
		jest.spyOn(database_models.Order, "findOne").mockResolvedValue(null);
		...
		expect(body.status).toStrictEqual("NOT FOUND");
		expect(body.message).toStrictEqual("Order not found");
	});
});
